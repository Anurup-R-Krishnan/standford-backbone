...
class NuSMV:
    ...
    def generate_nusmv_trans(self, tf, end_ports):
        ...
        for rule in tf.rules:
            ...
            if rule["match"]:
                for i in range(num_parts):
                    ...
                    nusmv_rule = (
                        nusmv_rule
                        + f" & ((h{i} & 0ub{num_bits}_{mask[l - e_range : l - s_range]}) = 0ub{num_bits}_{match[l - e_range : l - s_range]})"
                    )
            ...
            if rule["affected_by"]:
                for aff_rule, aff_intersect, aff_ports in rule["affected_by"]:
                    ...
                    for i in range(num_parts):
                        ...
                        affected = (
                            affected
                            + f" | ((h{i} & 0ub{num_bits}_{aff_mask[l - e_range : l - s_range]}) != 0ub{num_bits}_{aff_match[l - e_range : l - s_range]})"
                        )
            ...
            if rule["mask"] is not None and rule["rewrite"] is not None:
                ...
                for i in range(num_parts):
                    ...
                    nusmv_rule = (
                        nusmv_rule
                        + f" & (next(h{i}) = ((h{i} & 0ub{num_bits}_{action_mask[l - e_range : l - s_range]}) | 0ub{num_bits}_{action_rewrite[l - e_range : l - s_range]}))"
                    )
            else:
                for i in range(num_parts):
            ...
            if rule["action"] == "link":
                nusmv_rule = nusmv_rule + " & (next(pin) = 0ud%d_%d)" % (
                    PORT_VAR_LEN,
                    rule["out_ports"][0] + self.out_port_offset,
                )
            else:
        ...
        for end_port in end_ports:
            nusmv_rule = "p = 0ud%d_%s & next(p) = 0ud%d_%s" % (
                PORT_VAR_LEN,
                end_port,
                PORT_VAR_LEN,
                end_port,
        ...
        nusmv_rules.append(
            "p = 0ud%d_%s & next(p) = 0ud%d_%s"
            % (PORT_VAR_LEN, self.invalid_port, PORT_VAR_LEN, self.invalid_port)
        )
    ...
    def generate_nusmv_input(self):
        ...
        self.generated_nusmv_input = (
            self.generated_nusmv_input + f"p: unsigned word[{PORT_VAR_LEN}];\n"
        )
        self.generated_nusmv_input = (
            self.generated_nusmv_input + f"pin: unsigned word[{PORT_VAR_LEN}];\n"
        )
        num_parts = int(ceil(self.length / (HDR_VAR_LEN * 1.0)))
        for i in range(num_parts):
            i * HDR_VAR_LEN
            if self.length > i * HDR_VAR_LEN + HDR_VAR_LEN - 1:
                ...
                num_bits = self.length - HDR_VAR_LEN * i
            self.generated_nusmv_input = (
                self.generated_nusmv_input + f"h{i}: unsigned word[{num_bits}];\n"
            )
        self.generated_nusmv_input = self.generated_nusmv_input + "\n"
        self.generated_nusmv_input = (
            self.generated_nusmv_input + "INIT p = 0ud%d_0;\n" % PORT_VAR_LEN
        )
        self.generated_nusmv_input = (
            self.generated_nusmv_input + "INIT pin = 0ud%d_0;\n" % PORT_VAR_LEN
        )
