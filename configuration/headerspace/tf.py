...
class TF:
    ...
    def activate_hash_table(self, nibble_indices):
        ...
        for key in self.inport_to_rule:
    ...
    def activate_exact_match_hash(self, indices):
        ...
        for rule in self.rules:
            ...
            for in_port in in_ports:
                if "%d" % in_port not in self.exact_match_hash:
                    self.exact_match_hash["%d" % in_port] = {}
                if match_key_string not in self.exact_match_hash["%d" % in_port]:
    ...
    def print_influences(self):
        ...
        for rule in self.rules:
            print(
                f"{rule['action']} Rule Match: {byte_array_to_pretty_hs_string(rule['match'])},{rule['in_ports']}"
            )
            print("Affected by:")
            for aff in rule["affected_by"]:
                print(
                    f"{byte_array_to_pretty_hs_string(aff[0]['match'])}: On Ports {aff[2]}, Intersect= {byte_array_to_pretty_hs_string(aff[1])}"
                )
            print("Influence on:")
            ...
            if self.rules[i]["action"] == "rw" or self.rules[i]["action"] == "fwd":
                ...
                if len(intersect) > 0 and len(common_ports) > 0:
                    ...
                    self.rules[i]["affected_by"].append(
                        (self.rules[priority], intersect, common_ports)
                    )
    ...
    def set_fast_lookup_pointers(self, priority):
        ...
        for p in in_ports:
            port = "%d" % p
            if port not in self.inport_to_rule:
                self.inport_to_rule[port] = []
            self.inport_to_rule[port].append(new_rule)
        for p in out_ports:
            port = "%d" % p
            if port not in self.outport_to_rule:
    ...
    def add_rewrite_rule(self, rule, priority=-1):
        ...
        * 'out_ports': the list of output port numbers.
        ...
        extended_rule["inverse_rewrite"] = byte_array_and(
            byte_array_not(rule["mask"]), rule["match"]
        )
    ...
    def apply_rewrite_rule(self, rule, hs, port, applied_rules=None):
        ...
        if new_hs.count() > 0 and port in rule["in_ports"]:
            for i in range(len(new_hs.hs_list)):
                barr = byte_array_or(
                    byte_array_and(new_hs.hs_list[i], rule["mask"]), rule["rewrite"]
                )
            ...
            for i in range(len(new_hs.hs_diff)):
                barr = byte_array_or(
                    byte_array_and(new_hs.hs_diff[i], rule["mask"]), rule["rewrite"]
                )
        ...
        if default_operation == False:
            ...
            try:
                if len(match_key_string) == len(self.exact_match_indices) * 4:
                    # Full match
                    rule_set = (
                        self.exact_match_hash["%d" % port][match_key_string]
                        + self.exact_match_hash["%d" % port]["default"]
                    )
                    # print "Exact", len(self.exact_match_hash["%d"%port][match_key_string])
                    # print "Default", len(self.exact_match_hash["%d"%port]["default"])
                else:
                    for key in self.exact_match_hash["%d" % port]:
            ...
            # TODO: Hack! fix it
            if "%d" % port in self.inport_to_rule and (
                not self.hash_table_active or len(hs.hs_list) > 1
            ):
    ...
    def apply_inv_rewrite_rule(self, rule, hs, port):
        ...
        if new_hs.count() > 0:
            for i in range(len(new_hs.hs_list)):
                barr = byte_array_or(
                    byte_array_and(new_hs.hs_list[i], rule["mask"]),
                    rule["inverse_rewrite"],
                )
                new_hs.hs_list[i] = barr
            for i in range(len(new_hs.hs_diff)):
                barr = byte_array_or(
                    byte_array_and(new_hs.hs_diff[i], rule["mask"]),
                    rule["inverse_rewrite"],
                )
                new_hs.hs_diff[i] = barr
    ...
    def save_object_to_file(self, file):
        ...
        """
        print(f"=== Saving transfer function to file {file} ===")
        f = open(file, "w")
        f.write(
            "%d$%s$%d$%d$%d$\n"
            % (
    ...
    def load_object_from_file(self, file):
        ...
        """
        print(f"=== Loading transfer function from file {file} ===")
        f = open(file)
        self.rules = []
        ...
        for line in f:
            ...
            for affect in affect_list:
                if affect != "":
                    ...
                    new_affect = (
                        int(elems[0]),
                        hs_string_to_byte_array(elems[1]),
                        prts,
                    )
        ...
        print(f"=== Transfer function loaded from file {file} ===")
