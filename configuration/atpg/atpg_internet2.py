...
ntf_global = ""
ttf_global = ""
src_port_ids_global: set[int] = set()
dst_port_ids_global: set[int] = set()

DATABASE_FILE = "work/i2.sqlite"
...
port_reverse_map_global: dict[str, str] = {}
port_map_global: dict[str, dict[str, int]] = {}
...
def rule_lists_compress(rule_lists):
    ...
    print(
        "Global Compression: Start=%d, End=%d, Ratio=%f, Time=%f"
        % (start_packets, end_packets, float(end_packets) / start_packets, en - st)
    )
...
def find_test_packets(src_port_id):
    ...
    st = time.time()
    paths = find_reachability_test(
        ntf_global, ttf_global, src_port_id, dst_port_ids_global, test_pkt
    )
    en = time.time()

    print_paths_to_database(paths, port_reverse_map_global, TABLE_TEST_PACKETS)
    result_string = "Port:%d, Path No:%d, Time: %fs" % (
        src_port_id,
        len(paths),
        en - st,
    )
    ...
    result_string = "Port:%d, Compressed Path No:%d, Time: %fs" % (
        src_port_id,
        len(paths),
        en - st,
    )
...
def main():
    ...
    conn.execute(
        f"CREATE TABLE {TABLE_NETWORK_RULES} (rule TEXT, input_port TEXT, output_port TEXT, action TEXT, file TEXT, line TEXT)"
    )
    conn.execute(
        f"CREATE TABLE {TABLE_TOPOLOGY_RULES} (rule TEXT, input_port TEXT, output_port TEXT)"
    )
    conn.execute(
        f"CREATE TABLE {TABLE_TEST_PACKETS} (header TEXT, input_port INTEGER, output_port INTEGER, ports TEXT, no_of_ports INTEGER, rules TEXT, no_of_rules INTEGER)"
    )
    conn.execute(
        f"CREATE TABLE {TABLE_TEST_PACKETS_LOCALLY_COMPRESSED} (header TEXT, input_port INTEGER, output_port INTEGER, ports TEXT, no_of_ports INTEGER, rules TEXT, no_of_rules INTEGER)"
    )
    conn.execute(
        f"CREATE TABLE {TABLE_TEST_PACKETS_GLOBALLY_COMPRESSED} (rules TEXT, no_of_rules INTEGER)"
    )
