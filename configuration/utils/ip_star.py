...
def make_byte_array_ip_star_hdr(
    num_stack,
    stack_ip_list,
    stack_subnet_list,
    ip_src,
        ...
        for elem in elem_list:
            ...
            for i in range(8):
                ...
                for j in range(num_stack - 1):
                    elem[2 * format["stack_pos"] + i + j * 8] = elem[
                        2 * format["stack_pos"] + i + j * 8 + 8
                    ]
            for i in range(8):
        ...
        for elem in elem_list:
            ...
            for i in range(8):
                ...
                for j in reversed(range(num_stack - 1)):
                    elem[format["stack_pos"] * 2 + 8 * j + i + 8] = elem[
                        format["stack_pos"] * 2 + 8 * j + i
                    ]
                elem[2 * format["stack_pos"] + i] = b_tmp[i]
                elem[format["ip_dst_pos"] * 2 + i] = elem[
                    format["stack_pos"] * 2 + 8 * num_stack + i - 8
                ]
...
def make_NTF(num_mbox):
    ...
    for i in range(num_mbox):
        ...
        rule = TF.create_standard_rule(
            [1, 7], ip_match, [2], None, None, "sample.txt", [line_counter]
        )
    ...
    for i in range(num_mbox):
        ip_match = make_byte_array_ip_star_hdr(None, [], [], 0, 0, m_addr[i], 32)
        rule = TF.create_standard_rule(
            [3, 4], ip_match, [14 + 4 * i], None, None, "sample.txt", [line_counter]
        )
        NTF.add_fwd_rule(rule)
        line_counter += 1
    ...
    for i in range(num_mbox):
        rule = TF.create_custom_rule(
            make_IP_str_match(all_ip_star_ingress_ports),
    ...
    for i in range(num_mbox):
        rule = TF.create_custom_rule(
            make_mbox_match(m_addr[i], 32, 17 + 4 * i),
            make_mbox_tf(),
...
def expand_NTF(NTF, num_rules):
    port_groups = [
        [1, 2, 7, 101, 102, 103, 104, 105, 106, 107],
