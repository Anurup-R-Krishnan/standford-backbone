...
class juniperRouter:
    ...
    def acl_dictionary_entry_to_bytearray(self, dic_entry):
        ...
        tp_src_matches = range_to_wildcard(
            dic_entry["transport_src_begin"], dic_entry["transport_src_end"], 16
        )
        tmp = []
        for tp_src_match in tp_src_matches:
            ...
            tmp.append(b)
        result = tmp
        tp_dst_matches = range_to_wildcard(
            dic_entry["transport_dst_begin"], dic_entry["transport_dst_end"], 16
        )
        tmp = []
        for tp_dst_match in tp_dst_matches:
            for r in result:
                ...
                tmp.append(b)
        result = tmp
        tp_ctrl_matches = range_to_wildcard(
            dic_entry["tcp_ctrl_begin"], dic_entry["tcp_ctrl_end"], 8
        )
        tmp = []
    ...
    def acl_dictionary_entry_to_string(entry):
        ...
        output = (
            output
            + "ip protocol: %d -- src ip: %s -- src ip mask: %s -- src transport port: %d-%d -- dst ip: %s -- dst ip mask: %s -- dst transport port: %d-%d"
            % (
                entry["ip_protocol"],
                ...
                entry["transport_dst_begin"],
                entry["transport_dst_end"],
            )
        )
        return output
        ...
        for cnf_vlan in self.config_vlans:
            if "vlan%d" % cnf_vlan in self.vlan_ports:
                ...
                def_rule = TF.create_standard_rule(
                    all_in_ports, match, intermediate_port, None, None, "", []
                )
            ...
            for fwd_rule in self.fwd_table:
                ...
                if fwd_rule[1] != subnet:
                    ...
                    for output_port in fwd_rule[2]:
                        vlan = 0
                        m = re.split(r"\.", output_port)
                        # drop rules:
                        if output_port == "self":
                            self_rule = TF.create_standard_rule(
                                in_port, match, [], None, None, file_name, lines
                            )
                        ...
                        # non drop rules
                            ...
                            # vlan outputs
                                ...
                                if output_port in self.vlan_ports:
                                    ...
                                    print(f"ERROR: unrecognized vlan {output_port}")
                                    return -1
                            # physical ports - no vlan taging
                                ...
                                if output_port in self.port_to_id:
                                    ...
                                    print(f"ERROR: unrecognized port {output_port}")
                                    return -1
                        ...
                        tf_rule = TF.create_standard_rule(
                            in_port, match, out_ports, mask, rewrite, file_name, lines
                        )
            ...
            if line.startswith("Destination"):
                while True:
                    ...
                    if len(tokens) == 6 and tokens[3] == "indr":
                        ...
                        if port in self.ae_bundles:
                            ports_new = self.ae_bundles[port]
                            self.fwd_table.append(
                                [
                                    ip_subnet[0],
                                    ip_subnet[1],
                                    ports_new,
    ...
    def read_config_file(
        ...
        file_path,
        router_name,
        ns="{http://xml.juniper.net/junos/10.4R9/junos-interface}",
    ):
        ...
        for router in routers:
            if router.get("name") != router_name:
                continue
            physical_interfaces = router.findall(
                f"{ns}interface-information/{ns}physical-interface"
            )
            ...
            for physical_interface in physical_interfaces:
                ...
                for logical_interface in logical_interfaces:
                    ...
                    for address in addresses:
                        ...
                        vlan = int(re.split(r"\.", logical_interface_name)[1])
                        ...
                        if vlan >= 0 and vlan < 4096:
                            ...
                            self.port_subnets["%d" % vlan].append(
                                (
                                    ip_int,
                                    32 - mask_int,
                                    file_path,
