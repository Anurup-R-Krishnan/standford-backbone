...
class ciscoRouter:
    ...
    def acl_dictionary_entry_to_bytearray(self, dic_entry):
        ...
        tp_ctrl_matches = range_to_wildcard(
            dic_entry["transport_ctrl_begin"], dic_entry["transport_ctrl_end"], 8
        )
        tmp = []
    ...
    def read_route_file(self, file_path):
        ...
        for line in f:
            ...
            if is_ip_subnet(tokens[0]):
                ...
                if (
                    port.lower().startswith("loopback")
                    or port.lower().startswith("null")
                    or tokens[1].lower().startswith("drop")
                ):
                    port = "self"
    ...
    def generate_transfer_function(self, tf):
        ...
        for vlan_name in self.vlan_ports:
            ...
            if "vlan%d" % cnf_vlan in self.vlan_ports:
                ...
                if len(access_ports) > 0:
                    ...
                    def_rule = TF.create_standard_rule(
                        access_ports, match, intermediate_port, mask, rewrite, "", []
                    )
                    tf.add_fwd_rule(def_rule)
        ...
        if len(trunk_ports) > 0:
            ...
            def_rule = TF.create_standard_rule(
                list(trunk_ports), match, intermediate_port, mask, rewrite, "", []
            )
            tf.add_fwd_rule(def_rule)
