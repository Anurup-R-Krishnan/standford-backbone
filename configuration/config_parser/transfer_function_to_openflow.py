    ...
    def find_new_field(self, field_match, field_mask, field_rewrite):
        ...
        for i in range(len(new_byte_array)):
            for j in range(4):
                ...
                if next_bit == 0x03:
                    print(
                        f"ERROR: Unexpected rewrite action. Ignored. {byte_array_to_hs_string(field_match)} - {byte_array_to_hs_string(field_mask)} - {byte_array_to_hs_string(field_rewrite)} - {byte_array_to_hs_string(new_byte_array)}"
                    )
                    return None
                elif next_bit == 0x02:
                    value = value + 2 ** (4 * i + j)
