...
class cppizer:
    ...
    def __cancode(self, struct_in, coded):
        ...
        for member in struct_in.members:
            if isinstance(member, cheader.cstruct) and member.typename not in coded:
                result = False
        return result
                    ...
                    if member.object.typename == "char" and member.size != 0:
                        ...
                        for i in range(member.size):
                            if isinstance(member.object, cheader.cprimitive):
                                code.append(
                                    indent + self.tab + member.name + "[" + str(i) + "]"
                                    " = "
    ...
    def __determineprimtype(self, cprim):
        ...
        if (
            cprim.typename == "short"
            or cprim.typename == "unsigned short"
            or cprim.typename == "uint16_t"
        ):
            return "s"

        if (
            cprim.typename == "long"
            or cprim.typename == "unsigned long"
            or cprim.typename == "uint32_t"
        ):
            return "l"

        if (
            cprim.typename == "long long"
            or cprim.typename == "unsigned long long"
            or cprim.typename == "uint64_t"
        ):
            return "ll"
                                ...
                                code.append(
                                    ...
                                    + str(
                                        self.rules.get_default_value(
                                            member.object.typename, member.name
                                        )
                                    )
                                    + ";"
                                )
    ...
    def codemembers(self, struct_in, ccode):
        ...
        if not ccode:
            for member in struct_in.members:
                if (
                    isinstance(member, cheader.carray)
                    and member.size != 0
                    and member.object.typename == "char"
                ):
