...
class cstruct2py:
    ...
    def __init__(self):
        ...
        self.structmap["uint8_t"] = self.structmap["unsigned char"] = "B"
        self.structmap["short"] = "h"
        self.structmap["uint16_t"] = self.structmap["unsigned short"] = "H"
        self.structmap["int"] = "i"
        self.structmap["unsigned int"] = "I"
        self.structmap["long"] = "l"
        self.structmap["uint32_t"] = self.structmap["unsigned long"] = "L"
        self.structmap["long long"] = "q"
        self.structmap["uint64_t"] = self.structmap["unsigned long long"] = "Q"
...
class structpacker:
    ...
    def pack(self, ctype, *arg):
        ...
        if isinstance(ctype, str):
            ...
            return struct.pack(self.prefix + cstruct2py.get_pattern(ctype), *arg)
        else:
            return None
    ...
    def unpack_from_front(self, ctype, binaryString, returnDictionary=True):
        ...
        return (
            structpacker.peek_from_front(self, pattern, binaryString, returnDictionary),
            binaryString[dsize:],
        )
    ...
    def peek_from_front(self, ctype, binaryString, returnDictionary=True):
        ...
        # Return values
        if isinstance(ctype, str) or (not returnDictionary):
            return data
        else:
