...
class parser:
    ...
    def flow_mod_describe(self, packet, prefix=""):
        ...
        return (
            prefix
            + "Flow_mod of command "
            + self.__messages.get_enum_name("ofp_flow_mod_command", dic["command"][0])
            + " idle/hard timeout:"
            + str(dic["idle_timeout"][0])
            + "/"
            + str(dic["hard_timeout"][0])
    ...
    def match_describe(self, dic, nameprefix="", prefix=""):
        ...
        return (
            prefix
            + f"match wildcards:{dic[nameprefix + 'wildcards'][0]:x}"
            + " inport="
            + str(dic[nameprefix + "in_port"][0])
            + prefix
            + "     "
            + " ethertype="
            + str(dic[nameprefix + "dl_type"][0])
            + " vlan="
            + str(dic[nameprefix + "dl_vlan"][0])
            + " "
    ...
    def switch_config_describe(self, packet):
        ...
        return (
            "with flag "
            + str(self.__messages.get_enum_name("ofp_config_flags", dic["flags"][0]))
            + " and miss send length "
            + str(dic["miss_send_len"][0])
        )
