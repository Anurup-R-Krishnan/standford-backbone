...
ofsw = of_simu.switch(ofmsg, args[0], port, dpid=int("0xcafecafe", 16), parser=parser)
ofsw.send_hello()
# Send echo and wait
xid = 22092009
running = True
...
while running:
    ...
    if dic["type"][0] == ofmsg.get_value("OFPT_ECHO_REPLY") and dic["xid"][0] == xid:
        # Check reply for echo request
        output.info(
            "Received echo reply after " + str((pkttime - starttime) * 1000) + " ms",
            "ping-controller",
        )
        running = False
    else:
