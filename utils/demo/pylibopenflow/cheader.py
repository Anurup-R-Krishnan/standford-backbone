...
class cstruct(ctype):
    ...
    def expand(self, cheader):
        ...
        for member in self.members:
            if isinstance(member, cstruct) and (not member.expanded):
                try:
...
class carray(ctype):
    ...
    def __init__(self, typename, name, isPrimitive, size):
        ...
        ctype.__init__(self, typename, name, (isinstance(size, int) and isPrimitive))
        ##Object reference
        if isPrimitive:
...
class ctype_parser:
        ...
        if len(values) != 1:
            ...
            try:
                ...
                if val == "":
                    ...
                    sizeval = val
            return (sizeval, namepattern.findall(string)[0].strip()[0:-1])
    ...
    def parse_type(self, string):
        ...
        if len(parts) >= 2:
            ...
            if size != 1:
                # Array
                return carray(typename, name, self.is_primitive(typename), size)
            else:
                # Not array
