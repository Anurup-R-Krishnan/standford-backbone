...
class StanfordTopo(Topo):
    ...
    def generate_dummy_rules(self, dummy_switch_id, port_count):
        ingress = f"sudo ovs-ofctl add-flow s{dummy_switch_id} in_port=1,actions="
        for dst_port in range(2, port_count + 2):
            ingress += "output:%d," % dst_port
            self.dummy_rules.append(
                "sudo ovs-ofctl add-flow s%s in_port=%d,actions=output:1"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", dest="controller_port", type=int, default=6633, help="Controller's port"
    )
    parser.add_argument(
        "-c2",
        dest="dummy_controller_name",
        default="localhost",
        help="Dummy controller's hostname or IP",
    )
    parser.add_argument(
        "-p2",
        dest="dummy_controller_port",
        type=int,
        default=6633,
